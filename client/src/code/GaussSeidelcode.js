...
import gaussSeidel from '../utils/gaussSeidel';

function GaussSeidelcode() {
  const multiple = true
  ...
  const calculate = useCallback(() => {
      console.log(matrix)
      console.log(matrixY)
      const x  = gaussSeidel(matrix, matrixY)
      console.log(x)
      setResult(x)
      setShowResult(true)
  }, [matrix, matrixY])
            ...
            <TableBody>
              {result.map((item) => (
                ...
                >
                  ...
                  <TableCell size="small" component="td">{item.x_new.map((e,j) => {
                    return <List key={`x${item.i}${j}`}>{`x${j + 1} = ${parseFloat(e).toFixed(10)}`}</List>
                  })}
                  </TableCell>
                  <TableCell size="small" component="td">{(item.i === 1) ? '' : item.error.map((e, j) => {
                    return <List key= {`e${item.i}${j}`}>{`error x${j + 1} = ${parseFloat(e).toFixed(10)}`}</List>
                  })}
                  </TableCell>
                </TableRow>
              ))}
...
export default GaussSeidelcode