...
import { firstforwardH, secondforwardH, firstforwardH2, secondforwardH2 } from '../utils/forwardDividedDifference';
import axios from "axios"
import Plot from 'react-plotly.js';
const math = require('mathjs')

function ForwardDividedDifferencescode() {
    ...
    const handleSubmit = (e) => {
      e.preventDefault()
        ...
        if(alignmentOrder === 'first') {
            if(alignmentError === 'h') {
                k = firstforwardH(expr, 2, 0.25)
                console.log('firstforwardH')
            } else {
                k = firstforwardH2(expr, parseFloat(x), parseFloat(h))
                console.log('firstforwardH2')
            }
        }
        else {
            if(alignmentError === 'h') {
                k = secondforwardH(expr, parseFloat(x), parseFloat(h))
                console.log('secondforwardH')
            } else {
                k = secondforwardH2(expr, parseFloat(x), parseFloat(h))
                console.log('secondforwardH2')
            }
        }
        console.log(k)
        setResult(k)
...
export default ForwardDividedDifferencescode
